...
//	ReactionDao reactionDaoimpl;
	...
	public String update(ModelMap model, @PathVariable("id") int post_id, @RequestParam("image") MultipartFile image,
	...
	/*	if (user.getCurrentUsers().getRole() == 10 | user.getCurrentUsers().getRole() == 5
			...
			if (user.getCurrentUsers().getRole() == 10) {
			postDaoimpl.Update(new Posts(post_id, userDaoimpl.findByID(user_id), url, photo2, title, overview, making, material, time, 
			...
			return "redirect: /yummy-fpt/post-show/" + post_id + "";
	...
	public String create(ModelMap model, 
		...
		int id = postDaoimpl.Create(new Posts(userDaoimpl.findByID(user.getCurrentUsers().getId()), url,
				photo2, title, overview, making, material, time, ration, kind, level, price, nation, 
				holiday, category, suitable, timestamp, "null"));

		return "redirect: /yummy-fpt/post-show/" + id + "";
	}
